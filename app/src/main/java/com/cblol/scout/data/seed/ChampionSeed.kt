...
/**
 * **Dados de seed dos campeões.**
 *
 * Esta é a ÚNICA fonte hardcoded de campeões e existe apenas para popular o
 * banco Realm criptografado na primeira execução do app
 * (ver [com.cblol.scout.data.realm.RealmSeeder]). Depois do seed, o app lê
 * exclusivamente do Realm via [com.cblol.scout.domain.datasource.StaticDataSource].
 *
 * Não é consultada em runtime pela lógica do jogo — o `ChampionRepository`
 * agora delega ao DataSource. Mantemos os dados em Kotlin (em vez de JSON) para
 * preservar a checagem de tipos das tags no momento da compilação.
 *
 * Tags ordenadas por importância para o draft (as 3 primeiras são exibidas no
 * card). Baseado nas classes oficiais da Riot e no meta competitivo 2026.
 */
object ChampionSeed {

    /** Lista completa de campeões para seeding. */
    ...
    private val champions: List<Champion> = listOf(
